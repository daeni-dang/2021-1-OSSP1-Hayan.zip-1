            ...
            if ((string_table[i][k][0] =='은'and string_table[i][k][1] =='JX') or (string_table[i][k][0] =='는'and string_table[i][k][1] =='JX')):
                ...
                for x in range(len(string_table[i])):# 테이블의 i번째 문장 길이동안
                    if (string_table[i][x][1]=='JKS'): # jsk(주격 조사가 있으면)
                        jks_cnt +=1  #count변수++
                for jx in range(0, k):
                    if ((string_table[i][jx][0] =='은'and string_table[i][jx][1] =='JX') or (string_table[i][jx][0] =='는'and string_table[i][jx][1] =='JX')):
                        jx_cnt +=1
                if (jks_cnt<0 and jx_cnt<0): #만약 주격 조사가 없으면
                    ...
                    s_table.append(string_table[i][N_cnt])# 주어라고 저장
                    s_table.append(string_table[i][k])   #주어 뒤에 조사(확인용)
    print("주어:")
            ...
            if ((string_table[i][k][0] =='은'and string_table[i][k][1] =='JX') or (string_table[i][k][0] =='는'and string_table[i][k][1] =='JX')):
                ...
                for x in range(len(string_table[i])):  # 테이블의 i번째 문장 길이동안
                    if (string_table[i][x][1] == 'JKS'):  # jsk(주격 조사가 있으면)
                        jks_cnt += 1  # count변수++
                for jx in range(0, k):
                    if ((string_table[i][jx][0] == '은' and string_table[i][jx][1] == 'JX') or (
                            string_table[i][jx][0] == '는' and string_table[i][jx][1] == 'JX')):
                        jx_cnt += 1
                if (jks_cnt>=0 or jx_cnt>=0): #주격 조사가 있으면
                    ...
                    o_table.append(string_table[i][N_cnt])#조사 앞을 목적어라고 저장
                    o_table.append(string_table[i][k]) #목적어 뒤에 조사(확인용)
